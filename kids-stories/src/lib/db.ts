...
export function getParagraphs(storyId: string): ParagraphRow[] {
  return getDb()
    .prepare(
...
export function insertStory(story: Omit<StoryRow, "created_at" | "status" | "error_message">) {
  getDb()
    .prepare(
...
export function updateParagraphImage(
  storyId: string,
  position: number,
  imagePath: string
) {
...
export function updateStoryTitle(id: string, title: string) {
